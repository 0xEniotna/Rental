...
func __setup__() {
    %{
        ...
        context.renter = 2567890
        context.public_key = 12345
        context.renter_pubKey = 98765

        print("Deploy Contract") 
        context.rental_address = deploy_contract("./src/libraries/rental.cairo", { "owner": 1193046, "public_key": context.public_key}).contract_address
        print("rental address >>> " + str(context.rental_address))
        context.nft_address = deploy_contract("./src/libraries/nft_contract.cairo", { "name": 7235188, "symbol": 7235188, "owner": 1193046}).contract_address
        context.erc20_address = deploy_contract("./src/libraries/token.cairo", { "name": 1952805748, "symbol": 7631732, "decimals": 2, "initial_supply": {"low": 100000, "high": 0}, "recipient": context.renter, "owner": 1193046 }).contract_address
    %}
    return ();
}
...
func setup_rent_cancel_rugpull{
    ...
    %}
    INFT_contract.mint(contract_address=nft_address, to=rental_address, tokenId=Uint256(1,0));
    let (check_nft_minting) = INFT_contract.balanceOf(contract_address=nft_address, owner=rental_address);
    ...
    %{  
        stop_prank_callable = start_prank(context.owner, target_contract_address=context.rental_address)
    %}
    IRental.setWhitelistedToken(contract_address=rental_address, token_address=erc20_address);
