...
func __setup__() {
    %{
        ...
        print("erc20 address >>> " + str(context.erc20_address))
        ...
        print("nft address >>> " + str(context.nft_address))
...
func test_deposit_withdraw_nft{
    ...
    %{ 
        stop_prank_callable = start_prank(context.owner, target_contract_address=context.rental_address) 
        expect_events({"name": "TokenWithdrawal", "from_address": ids.rental_address})
    %}
...
func test_listing_unlisting{
    ...
    %{
        expect_events({"name": "TokenListed", "from_address": ids.rental_address})
    %}
    IRental.listRental(contract_address=rental_address, new_price= Uint256(10,0), duration=3600);
    ...
    with_attr error_message("Price should be 10 after listing") {
        ...
        assert price1.high = 0;
    }
    IRental.unlistRental(contract_address=rental_address);
    let (listed3) = IRental.isListed(contract_address=rental_address);
    with_attr error_message("NFT should not be listed") {
        assert listed3 = 0;
    }
    %{ stop_prank_callable() %}
    return ();
...
func setup_rent_time{
    ...
    %{  
        stop_prank_callable = start_prank(context.owner, target_contract_address=context.rental_address)
    %}
    IRental.listRental(contract_address=rental_address, new_price= Uint256(1000,0) , duration=3600);
    %{ stop_prank_callable() %}
...
func test_rent_time{
    ...
    %{ 
        stop_prank_callable = start_prank(context.renter, target_contract_address=context.rental_address)
        expect_events({"name": "TokenRented", "from_address": ids.rental_address})
    %}

    IRental.rent(contract_address=rental_address, public_key=renter_pubKey);
    %{ stop_prank_callable() %}
    let (balance) = IERC20.balanceOf(contract_address=erc20_address, account=rental_address);
    with_attr error_message("Balance is incorrect, rental should have 1000 tokens") {
        assert balance.low = 1000;
    ...
    %{ 
        stop_prank_callable = start_prank(context.owner, target_contract_address=context.rental_address)
        expect_revert(error_message="Rental not over") 
    %}
